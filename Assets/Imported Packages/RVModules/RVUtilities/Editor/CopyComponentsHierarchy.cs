...
using RVModules.RVUtilities.PrototypingAndTesting;
using UnityEditor;
using UnityEngine;
...
public class CopyComponentsHierarchy : MonoBehaviour
{
    ...
    public bool copySerializedManagedFieldsOnly = false;
    public bool copyComponents = false;
    public bool copyTransformLocals = false;

    [Tooltip("If true, will try to get component to apply it's values from source.")]
    public bool tryGetComponentOnTargetFirst = false;
    ...
    public void Work()
    {
        ...
        if (copyComponents)
            for (var index = 0; index < targetComponents.Count; index++)
            {
                ...
                foreach (var component in sourceTransform.GetComponents<Component>())
                {
                    CopyComponent(component, targetTransform);
                }
            }
    ...
    private void CopyComponent(Component component, Transform targetTransform)
    {
        ...
        bool includeThisComponent = true;
