...
namespace RVModules.RVUtilities
{
    public class TerrainTools : MonoBehaviour
    {
        ...
        public void RemoveOverlappingTrees()
        {
            ...
            List<TreeInstance> treesNoOverlapping = new List<TreeInstance>();

            Debug.Log("Before tree instances: " + tdata.treeInstanceCount);

            foreach (TreeInstance tree in tdata.treeInstances)
            {
                if (!CheckForOverlapping(positions, tree.position, tdata.size))
                {
                    treesNoOverlapping.Add(tree);
                }
        ...
        public void CreateTerrainUnevenness()
        {
            ...
            float[,] newHeightmap = new float[tdata.heightmapResolution, tdata.heightmapResolution];

            for (int x = 0; x < tdata.heightmapResolution; x++)
            {
                for (int y = 0; y < tdata.heightmapResolution; y++)
                {
                    float heightChangeValue = terrainUnevenness/tdata.size.y;
