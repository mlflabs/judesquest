...
namespace RVModules.RVSmartAI
{
    ...
    [Serializable] public partial class TaskHandler
    {
        ...
        /// <summary>
        /// Returns copy of tasks waiting queue
        /// </summary>
        public ILoadBalancedTask[] Queue => queue.Keys.ToArray();

        /// <summary>
        /// Returns copy of currently running tasks
        /// </summary>
        public ILoadBalancedTask[] RunningTasks => runningTasks.ToArray();
        ...
        /// </summary>
        public ILoadBalancedTask LowestPriorityQueuedTask => LowestPriorityTask(queue.Keys.ToList());

        /// <summary>
        /// Returns queued task with the highest priority
        /// Null if there are none queued tasks
        /// </summary>
        public ILoadBalancedTask HighestPriorityQueuedTask => HighestPriorityTask(queue.Keys.ToList());
        ...
        public bool ScheduleTask(ILoadBalancedTask _task, int _frequency) =>
            ScheduleTask(_task, new LoadBalancerConfig(LoadBalancerType.TimeIntervalPerSec, _frequency));
        ...
        {
            if (runningTasks.Any(_aiTask => _aiTask.Action == _task.Action)) return false;
        ...
        /// This is in TaskHandler context, so if you pass here running task that is not handled by this TaskHandler it will return false
        /// </summary>
        public bool IsTaskRunning(string taskName) => runningTasks.Any(balancedTask => balancedTask.Name == taskName);
        ...
        {
            var queuedTask = queue.Keys.FirstOrDefault(_task => _task.Name == taskName);
        ...
        {
            var runningTask = runningTasks.FirstOrDefault(_task => _task.Name == taskName);
        ...
        private ILoadBalancedTask LowestPriorityTask(List<ILoadBalancedTask> loadBalancedTasks)
        {
        ...
        private ILoadBalancedTask HighestPriorityTask(List<ILoadBalancedTask> loadBalancedTasks)
        {
            ILoadBalancedTask task = null;
            foreach (var activeTask in loadBalancedTasks)
                if (task == null || activeTask.Priority > task.Priority)
        ...
        private bool StartTaskInternal(ILoadBalancedTask _task, LoadBalancerConfig _frequency)
        {
            _task.OnTaskFinish += () =>
            {
                ...
                if (queue.Count > 0)
                {
                    var mostImpTaskFromQueue = queue.OrderBy(task => task.Key.Priority).ElementAt(0);
                    ScheduleTask(mostImpTaskFromQueue.Key, mostImpTaskFromQueue.Value);
                }
#if UNITY_EDITOR
                UpdateDebugData();
#endif
